...
async def spawn_avatar(room_name: str) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            print("📞 Starting LiveKit-Simli integration...")
            ...
            print("✅ Avatar connected to room successfully!")

            # Step 4: Keep function alive to maintain avatar session
            print("⏳ Avatar session active (will run for 15 minutes)...")
            await asyncio.sleep(900)

            print("✅ Avatar session completed")
...
async def spawn_avatar(room_name: str) -> None:
    ...
    try:
        ...
        try:
            ...
            print("🎭 Starting Simli avatar...")
            await simli_avatar.start(session, room=room)
            print("✅ Avatar started successfully")

            # Keep alive for session duration (900s = 15min)
            print("⏳ Avatar session active...")
            await asyncio.sleep(900)
...
async def main(context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Participant display name is currently fixed to "AI Interviewer" inside
        # spawn_avatar(); accept the override for forward compatibility.
        _participant_name = payload.get("participantName", "AI Interviewer")

        if not room_name:
            return context.res.json({"success": False, "error": "roomName is required"}, 400)
        ...
        return context.res.json(
            {"success": True, "message": f"Avatar session ended for room: {room_name}"}
        )
