  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs[0]) {
      ...
      if (!tabs[0].url.includes("linkedin.com/in/")) {
        showMessage(
          "Please navigate to a LinkedIn profile page to extract data",
          "warning",
        );
...
extractBtn.addEventListener("click", async () => {
  ...
  extractBtn.innerHTML =
    '<span class="loader"></span><span>Extracting...</span>';
  messageContainer.innerHTML = "";
  ...
  try {
    ...
    if (!tab.url?.includes("linkedin.com/in/")) {
      showMessage(
        "Not a LinkedIn profile page. Please navigate to a profile.",
        "error",
      );
    ...
    if (!scriptLoaded) {
      showMessage(
        "Failed to load on this page. Please refresh and try again.",
        "error",
      );
      extractBtn.disabled = false;
      extractBtn.innerHTML = originalText;
    ...
    const extractionPromise = new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        reject(
          new Error(
            "Extraction timed out. The page may be loading or content not found.",
          ),
        );
      ...
      chrome.tabs.sendMessage(
        ...
        (response) => {
          ...
          if (response?.success && response.data) {
            ...
            reject(
              new Error(response?.error || "Failed to extract profile data"),
            );
          }
        },
      );
  ...
  try {
    const sendPromise = new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        reject(
          new Error(
            "Request timed out. Is JobPrep running and accessible at " +
              targetUrl +
              "?",
          ),
        );
    ...
    if (result.success) {
      showMessage(
        "✅ Data sent successfully! Opening CV Builder...",
        "success",
        4000,
      );
      ...
      chrome.storage.local.set(
        { linkedInImportSessionId: result.sessionId },
        () => {
          // Check for storage errors
          if (chrome.runtime.lastError) {
            console.error(
              "Failed to store import session ID:",
          ...
          chrome.tabs.create({ url: cvBuilderUrl }, (tab) => {
            if (chrome.runtime.lastError) {
              ...
              showMessage(
                `Failed to open CV Builder: ${chrome.runtime.lastError.message}`,
                "error",
              );
            } else {
