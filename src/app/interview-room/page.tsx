...
import {
  TemplateManager,
  type Template,
} from "@/components/interview/template-manager";
...
export default function InterviewRoom() {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<
    Array<{ id: string; name: string; size: number; url: string }>
  >([] as Array<{ id: string; name: string; size: number; url: string }>);
  ...
  const handleJoinRoom = async () => {
    ...
    if (!hasMediaPermissions && !mediaPermissionsChecked) {
      toast.error("Please test your media setup first", {
        description:
          "Click 'Test Setup' to verify camera and microphone access",
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedRecent = [
          finalRoomName,
          ...recentRooms.filter((r) => r !== finalRoomName),
        ].slice(0, 5);
        setRecentRooms(updatedRecent);
        localStorage.setItem(
          "recentInterviewRooms",
        ...
        toast.success(
          `${joinMode === "create" ? "Room created" : "Joined room"} successfully!`,
        );
      } else {
    ...
    try {
      ...
      mediaRecorder.ondataavailable = (event) => {
        if (event.data.size > 0) {
          ...
          console.log(
            `Recorded chunk: ${event.data.size} bytes. Total chunks: ${recordedChunksRef.current.length}`,
          );
        }
      };
      ...
      mediaRecorder.onstop = () => {
        console.log(
          `Recording stopped. Total chunks collected: ${recordedChunksRef.current.length}`,
        );
      ...
      toast.success(
        "Recording started! Will be uploaded to cloud storage when stopped.",
      );
    } catch (error) {
  ...
  const handleDownloadTranscript = () => {
    const transcriptText = transcripts
      .map(
        (t) =>
          `[${t.timestamp.toLocaleTimeString()}] ${t.participantName}: ${t.text}`,
      )
      .join("\n");
  ...
  const handleInterviewGuide = () => {
    const guide = {
      ...
      category: "overall" as const,
      message:
        "Remember to: 1) Maintain eye contact 2) Ask follow-up questions 3) Take notes on key points 4) Allow candidate time to think 5) Be mindful of time",
      timestamp: new Date(),
      confidence: 1.0,
    };
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {isRecording && " Your recording will be stopped."} This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="pt-3 border-t border-border">
                    <div className="flex items-start gap-2 text-xs text-muted-foreground">
                      ...
                      <p>
                        Share the room code with participants to invite them to
                        your interview
                      </p>
                    </div>
                  </div>
  ...
  const serverUrl =
    ...
    "wss://jobprep-xundsxxf.livekit.cloud";
      ...
      <div className="min-h-screen bg-background text-foreground">
        <div className="container mx-auto p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 min-h-[calc(100vh-200px)]">
            ...
            {/* Main Video Area */}
            <div
              className={`${showSettings ? "lg:col-span-3" : "lg:col-span-3"} space-y-4`}
            >
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              disabled={
                                chatMessages.filter((m) => m.type === "note")
                                  .length === 0
                              }
                            >
                              <Download className="w-4 h-4" />
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Are you sure you want to leave this interview room?
                  {isRecording && " Your recording will be stopped."} This
            ...
            <DialogContent className="sm:max-w-[500px] z-[9999]">
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="rounded-lg border bg-muted/50 p-3 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Recipients will receive the room code and a direct link to
                    join
                  </p>
                </div>
              </div>
