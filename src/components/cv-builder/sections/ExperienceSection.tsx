...
function SortableExperienceItem({ exp, updateExperience, removeExperience }: SortableExperienceItemProps) {
	const {
		attributes,
		listeners,
		setNodeRef,
	...
	return (
		<Card ref={setNodeRef} style={style} className={`p-4 ${isDragging ? "shadow-2xl ring-2 ring-primary" : ""}`}>
		...
		<div className="space-y-4">
			...
			{experience.length === 0 ? (
				<Card className="p-8 text-center">
					<p className="text-muted-foreground">No experience added yet. Click "Add Experience" to start.</p>
				</Card>
			) : (
				<DndContext
					sensors={sensors}
