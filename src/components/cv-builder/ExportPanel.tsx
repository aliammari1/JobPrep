...
export function ExportPanel() {
	...
	const [selectedFormat, setSelectedFormat] = useState<"pdf" | "docx" | "json" | "txt" | "linkedin">("pdf");
	...
	const handleExport = async () => {
		setIsExporting(true);
		const loadingToast = toast.loading(`Generating ${selectedFormat.toUpperCase()}...`);
	...
	const handleEmailExport = async () => {
		...
		const blob = await res.blob();
		const file = new File([blob], `${cvData.personalInfo.fullName}_Resume.pdf`, { type: "application/pdf" });
		
		// Open mailto with attachment (note: not all email clients support this)
		const mailtoLink = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
		window.location.href = mailtoLink;
		
		toast.info("Email client opened. Please attach the downloaded PDF manually.");
	};
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent className="max-w-3xl">
							...
							<div className="grid gap-3">
								{exportFormats.map((format) => (
									<Card
										...
										onClick={() => !format.comingSoon && setSelectedFormat(format.id as any)}
									>
										<div className="flex items-start gap-4">
											...
											<div className="flex-1">
												...
												<p className="text-xs text-muted-foreground mt-1">Size: {format.size}</p>
											</div>
											{selectedFormat === format.id && (
							...
							<div className="grid gap-3 grid-cols-2">
								{["modern", "classic", "minimal"].map((template) => (
									<Card
										key={template}
										className={`p-4 cursor-pointer transition-all ${
											selectedTemplate === template ? "ring-2 ring-primary shadow-md" : "hover:shadow-sm"
										}`}
										onClick={() => setSelectedTemplate(template as any)}
									>
										<div className="flex items-center gap-3">
											<RadioGroupItem value={template} id={template} />
											<Label htmlFor={template} className="font-medium cursor-pointer capitalize">
												{template}
											</Label>
										</div>
				...
				<div className="flex items-center justify-between pt-4 border-t">
					<Button variant="outline" onClick={handleEmailExport} disabled={isExporting}>
